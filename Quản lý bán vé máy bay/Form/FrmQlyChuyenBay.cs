...
{
    public partial class FrmQlyChuyenBay : Form
    {
        #region Properties
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dtpkThoiGianKhoiHanh.Value < DateTime.Now)
                {
                    MessageBox.Show("Thời gian phải lớn, bằng thời gian hiện tại!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                int result = DateTime.Compare(dtpkThoiGianDen.Value, dtpkThoiGianKhoiHanh.Value);
                if (result <= 0)
                {
                    MessageBox.Show("Thời gian đến phải lớn hơn thời gian khởi hành!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                int result1 = DateTime.Compare(dtpkThoiGianDen.Value, (dtpkThoiGianKhoiHanh.Value).AddMinutes(Int32.Parse(txtThoiGianBay.Text)));
                if (result1 != 0)
                {
                    MessageBox.Show("Thời gian bay không khớp!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                try
                {
                    dtoChuyenBay = new DTO_ChuyenBay(txtMaChuyenBay.Text, cboMaTuyenBay.Text, cboMaMayBay.Text, dtpkThoiGianKhoiHanh.Value, dtpkThoiGianDen.Value,float.Parse(txtThoiGianBay.Text));
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            if (txtMaChuyenBay.Text.Trim() != "")
            {
                if (cboMaTuyenBay.Text.Trim() != "" && cboMaMayBay.Text.Trim() != "" && txtThoiGianBay.Text.Trim() != "")
                {
                    if (dtpkThoiGianKhoiHanh.Value < DateTime.Now)
                    {
                        MessageBox.Show("Thời gian đi phải lớn, bằng thời gian hiện tại!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                    int result = DateTime.Compare(dtpkThoiGianDen.Value, dtpkThoiGianKhoiHanh.Value);
                    if (result<=0)
        ...
        private void dtgvChuyenBay_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else
            {
                ...
                dtpkThoiGianDen.Value = Convert.ToDateTime(row.Cells[4].Value.ToString());
                txtThoiGianBay.Text = row.Cells[5].Value.ToString();
                cboMaTuyenBay_SelectionChangeCommitted(sender, e);
        ...
        private void txtMaChuyenBay_TextChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                cboSanBayDi.Text = row["TENSANBAYDI"].ToString();
                cboSanBayDen.Text = row["TENSANBAYDEN"].ToString();
                cboMaMayBay.Text = row["MAMAYBAY"].ToString();
                dtpkThoiGianKhoiHanh.Text = row["THOIGIANKHOIHANH"].ToString();
                dtpkThoiGianDen.Text = row["THOIGIANKHOIHANH"].ToString();
